-- Storage-backed PDF RAG: pgvector + knowledge_chunks + similarity RPC + message citations.
-- Chunks store storage_bucket / storage_path in metadata; signed URLs are built at query time in the app.
...
drop policy if exists "Service role full access knowledge chunks" on public.knowledge_chunks;
...
create index if not exists idx_knowledge_chunks_storage_path
  on public.knowledge_chunks ((metadata ->> 'storage_path'));

create or replace function public.match_knowledge_chunks(
  query_embedding vector(1536),
  match_threshold float default 0.22,
  match_count int default 28
)
returns table (
  id uuid,
  content text,
